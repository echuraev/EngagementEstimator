...
def open_tf_model(path):
    ...
    shape_dict = {
        "pnet/input": (N, H, W, 3),
        "rnet/input": (N, 24, 24, 3),
        "onet/input": (N, 48, 48, 3),
    }
    #mod, params = relay.frontend.from_tensorflow(gd, layout=layout, shape=shape_dict)
    #print(mod)
    #exit(123)
    #exit(234)
    sess = tf.compat.v1.Session(graph=full_graph)#,config=tf.ConfigProto(device_count={'CPU':1,'GPU':0}))
    pnet, rnet, onet = load_mtcnn(sess,full_graph)
    tf.saved_model.save(pnet, "pnet")
    tf.saved_model.save(rnet, "pnet")
    tf.saved_model.save(onet, "pnet")
    exit(234568)
    #print(type(pnet))
    #print(pnet)
    #print(tf.autograph.to_code(pnet))
    #print(pnet.get_concrete_function())
    layout = "NHWC"
    N = 1
    H = 1024
    W = 1024
    ...
    mod, params = relay.frontend.from_tensorflow(pnet, layout=layout, shape=shape_dict)
